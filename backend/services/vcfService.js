...
const parseVCF = async (filePath) => {
    ...
    // Check file extension for DOCX support
    if (filePath.endsWith('.docx')) {
        try {
            const result = await mammoth.extractRawText({ path: filePath });
            lines = result.value.split('\n'); // Raw text split by newline
        } catch (error) {
            console.error("Error reading DOCX:", error);
            throw new Error("Failed to read DOCX file content");
        }
    } else {
        // Standard VCF/TXT Read Stream
    ...
    console.log(`[VCF Parser] Extracted ${lines.length} lines from ${filePath}`);
    if (lines.length > 0) {
